...
let currentResponseElement = null; // Track the current response element for streaming

function addResponseMessage(message, isGeminiImageResponse = false, imageDataURL = null) {
    if (!currentResponseElement) {
        currentResponseElement = $(".message-bubble .response").last();
        currentResponseElement.empty(); // Clear only on initial call for a new message
    }
    ...
    if (isGeminiImageResponse && imageDataURL) {
        ...
        currentResponseElement.append('<div class="message-text">' + message + `<img src="${imageDataURL}" style="max-width: 30%; max-height: 30%; display: block; margin-top: 10px;" alt="Generated Image"></div>` + '<button class="view-button"><i class="fas fa-search"></i></button>' + '<button class="delete-message-btn"><i class="far fa-trash-alt"></i></button>');
         // 绑定查看按钮事件 (for image, but URL might be dataURL, so view button might not be very useful here unless you have original URL)
        currentResponseElement.find('.view-button').on('click', function() {
            // For data URL, opening in new tab might not be ideal, consider other actions if needed.
            window.open(imageDataURL, '_blank');
        });
        // 绑定删除按钮点击事件
        currentResponseElement.find('.delete-message-btn').click(function() {
            $(this).closest('.message-bubble').remove();
        });
         currentResponseElement = null; // Reset for next message
        ...
        if (message.startsWith('"//')) {
            ...
            currentResponseElement.append('<div class="message-text">' + '<audio controls=""><source src="data:audio/mpeg;base64,' + base64Data + '" type="audio/mpeg"></audio> ' + '</div>' + '<button class="delete-message-btn"><i class="far fa-trash-alt"></i></button>');
        } else if (message.startsWith('//')) {
            // 处理包含base64编码的音频
            const base64Data = message;
            currentResponseElement.append('<div class="message-text">' + '<audio controls=""><source src="data:audio/mpeg;base64,' + base64Data + '" type="audio/mpeg"></audio> ' + '</div>' + '<button class="delete-message-btn"><i class="far fa-trash-alt"></i></button>');
        } else {
            currentResponseElement.append('<div class="message-text">' + messageContent + '</div>' + '<button class="copy-button"><i class="far fa-copy"></i></button>');
            viewButtons.forEach(button => {
                currentResponseElement.append(button);
            });
            currentResponseElement.append('<button class="delete-message-btn"><i class="far fa-trash-alt"></i></button>');
        }
        ...
        currentResponseElement.find('.copy-button').click(function() {
            copyMessage($(this).prev().text().trim());
        });
        currentResponseElement.find('.delete-message-btn').click(function() {
            $(this).closest('.message-bubble').remove();
...
if (getCookie('streamOutput') !== 'false') { // 从 Cookie 获取流式输出设置, 默认流式
    ...
    while (true) {
        ...
        if (done) {
            currentResponseElement = null; // Reset currentResponseElement when stream is done
            break;
        }
